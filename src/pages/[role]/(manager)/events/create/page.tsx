...
const CreateEventPage = () => {
   ...
   const handleCreateEvent = async (data: FormValue) => {
      ...
      toast.promise(createEvent({ ...data, user_id: data?.user_id ?? user?.id, banner }).unwrap(), {
      ...
      <Form {...form}>
         <form className='flex max-w-4xl flex-col gap-y-14 sm:max-w-full' onSubmit={form.handleSubmit(handleCreateEvent)}>
            ...
            <Box className='space-y-10'>
               <Box className='grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-1 sm:gap-x-0'>
                  ...
                  <Box className='col-span-full w-full'>
                     <SelectFieldControl
                        ...
                        description='Khu vực tỉnh/thành phố nơi diễn ra sự kiện.'
                        options={areaOptions}
                     />
                  </Box>
                  <Box className='col-span-full w-full'>
